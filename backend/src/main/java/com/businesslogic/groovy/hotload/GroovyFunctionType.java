 ...
 * Groovy 自定义函数类型枚举
 *
 * <p>三种类型在注册流程、编译方式、Closure 包装方式上均有差异，
 * 详见 {@link GroovyFunctionRegistry} 的注册方法。
 *
 * <p>关联体系：
 * <ul>
...
public enum GroovyFunctionType {
     ...
     *
     * <p>直接编译多行脚本（参数由调用方在 call() 时注入 Binding），由
     ...
     *
     * <p>用于在 Groovy 脚本无法表达（如涉及 IO、外部服务调用）的场景下，桥接 Java 逻辑。
